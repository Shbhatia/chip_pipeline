...
x=read.table("RNASeq_noNA.txt")
library(graphics)
down=read.table("../overlap/down_overlap_genenames.bed")
down=down$V1
down.ix=which(rownames(y) %in% as.character(down))

up=read.table("../overlap/up_overlap_genenames.bed")
up=up$V1
up.ix=which((rownames(y) %in% as.character(up)) & abs(y$log2FoldChange)>1 )

postscript("volcano_shT12_1.ps", width= 50, height= 50)
 smoothScatter(y$log2FoldChange,-log10(x$padj),xlab=expression('Log'[2]*' Fold Change'),ylab=expression('-Log'[10]*' P-values'))
 p.ix= y$log2FoldChange>1 & y$padj<0.05
 points(y$log2FoldChange[p.ix],-log10(y$padj[p.ix]),col="goldenrod4", pch = 16)
 p.ix= y$log2FoldChange<(-1) & y$padj<0.05
 points(y$log2FoldChange[p.ix],-log10(y$padj[p.ix]),col="darkblue", pch = 16)
 abline(v=1, col ='grey',lty = 2);
 abline(v=(-1), col ='grey',lty = 2);
 abline(h=-log10(.05), col ='grey',lty = 2)
 points(y$log2FoldChange[up.ix],-log10(y$padj[up.ix]),pch = 4,col="red")
 points(y$log2FoldChange[down.ix],-log10(y$padj[down.ix]), pch = 4,col="red")
